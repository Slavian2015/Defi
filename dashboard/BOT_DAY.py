...
def new_data():
    ...
    dbrools.clean()
    dbrools.update_main_data(data=my_data)
...
def ref_data():
    ...
    if len(my_data) > 2800:
        del my_data[sorted(my_data.keys())[0]]
...
def my_sell():
    ...
    print("\nSELL ETHUP >>>>>>>>", round(my_usdt, 3))

    # data_status = {
    #     "date": datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3],
    #     "direct": "SELL",
    #     "kurs": round(my_kurs, 2),
    #     "balance": round(my_usdt, 2)}
    # dbrools.insert_history(data_status)
...
def my_buy():
    ...
    if my_usdt > 20:
        ...
        amount = (my_usdt - (my_usdt / float(my_kurs) * 0.005 * float(my_kurs))) / float(my_kurs)
...
def signal():
    ...
    # print("DIRECTION :", df["ttt"].iloc[-1], df["zero"].iloc[-1])

    def run():
        global start_price
        if start_price:
            if df["ttt"].iloc[-1] < df["zero"].iloc[-1]:
                    # mp = (df["close"].iloc[-1] - start_price) / start_price
                    # bot_message = f"\n DayTrader \n\nStart : {start_price}\nclose : {df['close'].iloc[-1]}\nProfit : {round(mp * 100, 2)}"
